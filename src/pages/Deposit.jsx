...
const Deposit = () => {
  ...
  const searchAccount = async () => {
    ...
    try {
      ...
      if (account.accountNumber) {
        ...
        setSuccess(
          `Account found: ${account.accountType} Account - ${account.accountNumber}`,
        );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await apiService.makeTransfer(depositData);

      if (response.data.statusCode === 200) {
        setSuccess(
          `Successfully deposited $${formData.amount} to account ${formData.accountNumber}`,
        );
      ...
      setError(
        error.response?.data?.message || 'An error occurred during deposit',
      );
    } finally {
      setLoading(false);
    }
              ...
              {accountInfo && (
                <div className="account-info">
                  ...
                  <div className="account-details">
                    <p>
                      <strong>Type:</strong> {accountInfo.accountType}
                    </p>
                    <p>
                    ...
                    <p>
                      ...
                      <span
                        className={`status ${accountInfo.status.toLowerCase()}`}
                      >
                        {accountInfo.status}
                      </span>
                    </p>
          ...
          <div className="transactions-card">
            ...
            {recentTransactions.length > 0 ? (
                    ...
                    <div key={transaction.id} className="transaction-item">
                      ...
                      <div className="transaction-details">
                        ...
                        <div className="transaction-account">
                          Account:{' '}
                          {transaction.sourceAccount || formData.accountNumber}
                        </div>
                      </div>
                    </div>
                  ))}
              ...
              <div className="no-transactions">
                ...
                <p className="hint">
                  Recent deposits will appear here after transactions
                </p>
              </div>
            )}
          </div>
